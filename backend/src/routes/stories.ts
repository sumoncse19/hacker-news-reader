...
const storiesQuerySchema = z.object({
  ...
  limit: z.coerce.number().int().min(1).max(50).default(config.hn.defaultPageSize),
});

// GET /api/stories
storiesRouter.get("/", async (req: Request, res: Response, next: NextFunction) => {
  try {
...
storiesRouter.get("/:id/comments", async (req: Request, res: Response, next: NextFunction) => {
  try {
    const id = parseInt(req.params.id as string, 10);
    if (isNaN(id)) {
      throw new ValidationError("Invalid story ID");
